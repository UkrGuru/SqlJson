...
public class DbFileTests
{
    ...
    public async Task CanSaveBinFile()
    {
        ...
        var guid = await DbHelper.ExecAsync<string?>("WJbFiles_Ins", new DbFile { FileName = filename, FileContent = GlobalTests.TestBytes5k });

        var file = await DbHelper.ExecAsync<DbFile>("WJbFiles_Get", guid);

        Assert.Equal(filename, file?.FileName);

        Assert.Equal(GlobalTests.TestBytes5k, file?.FileContent);
